...
from .douyin_signatures import VerifyFpManager
...
def ensure_douyin_web_cookies(ie, video_id, user_agent):
    ...
    if not ie._get_cookies(_DOUYIN_HOME).get("s_v_web_id"):
        ...
        # valid-format value ourselves using f2's algorithm.
        if not ie._get_cookies(_DOUYIN_HOME).get("s_v_web_id"):
            generated_id = VerifyFpManager.gen_s_v_web_id()
            ie._set_cookie(".douyin.com", "s_v_web_id", generated_id)
            ie.to_screen(f"Generated s_v_web_id: {generated_id}")
